...
exports.exportOrders = catchAsync(
  async (req, res, next) => {
    ...
    downloadOrders.rows.forEach(order => {
      ...
      downloadCandidate.forEach(candidate => {
        ...
        if (
          ...
          order.recipient =
            +customUserforDistrict?.tariff / 1000;
        }
        if (order.regionId === 1) {
          if (customUserforRegion) {
            order.recipient =
              +customUserforRegion.tariff / 1000;
      ...
      regionsJSON.forEach(region => {
        if (order.regionId == region.id) {
          order.regionId = region.name.slice(
            0,
      ...
      districtsJSON.forEach(district => {
        if (order.districtId == district.id) {
          order.districtId = district.name.slice(
            0,
            district.name.length - 7
          );
        }
      });
    });
    const ordersArr = Object.values(
      downloadOrders.rows.map(e => e)
    );
    let counter = 1;
    worksheet.addRow();
...
exports.getStatistics = catchAsync(
  async (req, res, next) => {
    ...
    let incomeSum =
      discountDeliveryPrice - discountTariff * soldOrders;
    let today = new Date();
