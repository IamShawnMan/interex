...
exports.createOrder = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new AppError(
        "Bu raqamli buyurtma mavjud. Takrorlansinmi?",
        400
      )
    );
  }
...
exports.changeStatusDeliveredOrders = catchAsync(
  async (req, res, next) => {
    ...
    const { id } = req.params;
    const { orderStatus, note, expense, quantity, price } =
      req.body;
    ...
    if (
      postOrdersById.orderStatus === "SOLD" &&
      quantity &&
      price
    ) {
      const halfOrderPrice =
        +postOrdersById.totalPrice - price;
      const halfOrderQuantity =
        +postOrderItemsById.quantity - quantity;
      ...
      const halfOrder = await Order.create({
        ...
        recipientPhoneNumber:
          postOrdersById.recipientPhoneNumber,
        orderStatus: statusOrder.STATUS_REJECTED,
        orderStatusUz: statusOrderUz.STATUS_OTKAZ,
...
exports.penidingRejactedReasons = catchAsync(
  async (req, res, next) => {
