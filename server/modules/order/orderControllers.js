...
exports.createOrder = catchAsync(async (req, res, next) => {
	...
	if (exisOrder === null || (req.query.phone === "free" && exisOrder)) {
		...
		order.orderItems?.forEach((item) => {
			itemByNote =
				itemByNote +
				`${item.productName?.replace(/ /g, "_")}-${+item.quantity}-dona`;
			items.push({
		...
		return next(
			new AppError("Bu raqamli buyurtma mavjud. Takrorlansinmi?", 400)
		);
	}
});
...
exports.changeOrderStatus = catchAsync(async (req, res, next) => {
	...
	if (userRole === rolesUser.ADMIN) {
		...
		const existedPackage = await Package.findByPk(orderById.packageId);
...
exports.deleteOrder = catchAsync(async (req, res, next) => {
	...
	if (orderById.orderStatus !== statusOrder.STATUS_NEW) {
		return next(new AppError("Buyurtmani o`chirib bo`lmaydi"));
	}
	packageByOrderId.packageTotalPrice =
		packageByOrderId.packageTotalPrice - orderById.totalPrice;
	...
	if (isNewOrdersInPackage === 0) {
		packageByOrderId.packageStatus = statusPackage.STATUS_OLD;
		packageByOrderId.packageStatusUz = statusPackageUz.STATUS_ESKI;
	} else {
		packageByOrderId.packageStatus = statusPackage.STATUS_NEW;
		packageByOrderId.packageStatusUz = statusPackageUz.STATUS_YANGI;
	}
	await packageByOrderId.save();
	res.json({
		status: "success",
...
exports.getMyOrders = catchAsync(async (req, res, next) => {
	...
	let myOrders = await Order.findAndCountAll(queryBuilder.queryOptions);
