...
router.get("/statisticcount", reportController.countsInRegionsAndMonths);
router
	.get(
		"/delivered",
	...
	.get("/delivered/reasons", orderControllers.penidingRejactedReasons)
	.get(
		"/delivered/daily",
		roleMiddleware([COURIER]),
...
router.route("/status").get(orderControllers.getAllOrderStatus);
...
router
	...
	.delete(roleMiddleware([STORE_OWNER]), orderControllers.deleteOrder);

router.route("/:id/edit").get(orderControllers.editOrder);

router
	.route("/:id/devprice")
	.patch(roleMiddleware([ADMIN]), orderControllers.changeDevPrice);
