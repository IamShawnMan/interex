...
function OrderMutation() {
 ...
 const navigate =useNavigate()
  useEffect(() => {
    getAllRegions()
    getAllDistrict()
    append({recipient:"",note:"",recipientPhoneNumber:"",regionId:"",districtId:"",orderItems:[]})
  },[])
  ...
  return (
    <form onSubmit={handleSubmit(data =>formSubmit(data))}>
      <ul>
        ...
        {fields.map((item, index) => (
          <li style={{border: "1px solid black"}}  key={item.id}>
            <input placeholder="recipient" {...register(`orders.${index}.recipient`)} />
             {errors?.orders?.[index]&&<>{errors?.orders?.[index]?.recipient?.message}</>}
            <input placeholder="note" {...register(`orders.${index}.note`)} />
            {errors?.orders?.[index]&&<>{errors?.orders?.[index]?.note?.message}</>}
            <input type="number" placeholder="phoneNumber" {...register(`orders.${index}.recipientPhoneNumber`)} />
            {errors?.orders?.[index]&&<>{errors?.orders?.[index]?.recipientPhoneNumber?.message}</>}
            ...
            {errors?.orders?.[index]&&<>{errors?.orders?.[index]?.regionId?.message}</>}
            ...
            {errors?.orders?.[index]&&<>{errors?.orders?.[index]?.districtId?.message}</>}
            <OrderItems orderIndex={index} control={control} errors={errors} register={register}/>
      ...
      <button
        type="button"
        onClick={() => append({recipient:"",note:"",recipientPhoneNumber:"",regionId:"",districtId:"",orderItems:[]})}
      >
        Add Order
      </button>
      <input type="submit" />
